...
Public Class Sales
    ...
    Dim CustID As Long
    Dim ID As Long
    Dim CusmrName, CusmrCNIC As String
    ...
    Private Sub CB_CName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_CName.SelectedIndexChanged
        CB_CNIC.SelectedItem = Nothing
        TB_TBalance.Text = ""
        TB_PrevBalance.Text = ""
    End Sub

    Private Sub CB_CNIC_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_CNIC.SelectedIndexChanged
        TB_TBalance.Text = ""
        TB_PrevBalance.Text = ""
    End Sub

    Private Sub CB_CNIC_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_CNIC.Enter
        CustCNIC()
    End Sub
    ...
    Private Sub TB_AmntPad_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub TB_ChqPad_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If TB_GRNTA.Text = "" Then
            GRNTA = 0
        Else
            GRNTA = TB_GRNTA.Text.Trim()
    ...
    Private Sub TB_PrevBalance_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If CB_CName.SelectedItem = Nothing Or CB_CNIC.SelectedItem = Nothing Then
            MessageBox.Show("Please select Customer/CNIC.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Hand)
        Else
            CustID = CusID(CB_CName.SelectedItem, CB_CNIC.SelectedItem)
            TB_PrevBalance.Text = PrevBlnc(CustID)
        End If
    End Sub

    Private Sub TB_TCredit_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub Btn_Submit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Submit.Click
        Try
            If CB_CName.SelectedItem = Nothing Then
                MessageBox.Show("Please select ""Customer Name"".", "Message", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            ElseIf CB_CNIC.SelectedItem = Nothing Then
                MessageBox.Show("Please select ""Customer CNIC"".", "Message", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            ...
            Else
                Try
                    ...
                    Using cmd = New SqlCommand("Select * from SR where CID = '" & CustID & "' and Invoice_No= '" & TB_IN.Text.Trim() & "' and Invoice_Date = '" & DTP_Inv_Date.Text & "'", con)
                    ...
                    Using cmd As New SqlCommand
                        ...
                        Try
                            cmd.CommandText = "Update SRPayment set Credit_Status = 'Idle' where Invoice_ID = (Select SR.Invoice_ID FROM SR INNER JOIN SRPayment ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE SR.CID = '" & CustID & "' AND SRPayment.Credit_Status = '" & CStatus.Text & "')"
                            cmd.ExecuteNonQuery()

                            cmd.CommandText = "Insert into SR (Invoice_ID, CID, Invoice_No, Invoice_Date, Picture) VALUES ('" & TB_GRN.Text.Trim() & "', '" & CustID & "', '" & TB_IN.Text.Trim() & "', '" & DTP_Inv_Date.Text & "', @Picture)"
    ...
    Private Sub Btn_UD_Load_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_UD_Load.Click
        ...
        Else
            ...
            cmd = New SqlCommand("Select * from SR where Invoice_ID='" & TB_UD_Load.Text.Trim() & "'", con)
            ...
            If dr.Read() Then
                ...
                Try
                    ...
                    Dim SplyerID As Long = CIDByGRN(TB_UD_Load.Text.Trim())
                    Using cmd = New SqlCommand("select Invoice_ID from SR where Invoice_ID > '" & TB_UD_Load.Text.Trim() & "' and CID = '" & SplyerID & "'", con)
                        If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                        ...
                        End Using
                    End Using
                    Using cmd = New SqlCommand("select * from SRItems where Invoice_ID = '" & TB_UD_Load.Text.Trim() & "' and Return_Status = '" & RStas & "'", con)
                        If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                        Dim count As Integer = 0
                        Using dr = cmd.ExecuteReader()
                            If dr.Read() Then
                        ...
                        End Using
                    End Using
                    Using cmd = New SqlCommand("select Invoice_ID from SRPayment where Invoice_ID = '" & TB_UD_Load.Text.Trim() & "'", con)
                        If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                        Dim count As Integer = 0
                        Using dr = cmd.ExecuteReader()
                            While dr.Read()
                        ...
                        Try
                            ...
                            Using dr = cmd.ExecuteReader()
                                If dr.Read() Then
                                    ...
                                    UD_TB_GRNTA.Text = dr.Item("SRTAmount")
                            ...
                            UD_CB_CName.SelectedItem = CusmrName
                            CustCNIC()
                            UD_CB_CNIC.SelectedItem = CusmrCNIC
    ...
    Private Sub UD_CB_CName_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UD_CB_CName.SelectedIndexChanged
        UD_CB_CNIC.SelectedItem = Nothing
        UD_TB_TBalance.Text = ""
    ...
    Private Sub UD_CB_CName_SelectionChangeCommitted(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UD_CB_CName.SelectionChangeCommitted
        UD_CB_CNIC_SelectionChangeCommitted(sender, e)
    End Sub

    Private Sub UD_CB_CNIC_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UD_CB_CNIC.SelectedIndexChanged
        UD_TB_TBalance.Text = ""
        UD_TB_PrevBalance.Text = ""
    End Sub

    Private Sub UD_CB_CNIC_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UD_CB_CNIC.Enter
        CustCNIC()
    End Sub

    Private Sub UD_CB_CNIC_SelectionChangeCommitted(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles UD_CB_CNIC.SelectionChangeCommitted
    ...
    Private Sub UD_TB_AmntPad_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub UD_TB_ChqPad_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If UD_TB_GRNTA.Text = "" Then
            GRNTA = 0
        Else
            GRNTA = UD_TB_GRNTA.Text.Trim()
    ...
    Private Sub UD_TB_PrevBalance_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)

        If UD_CB_CName.SelectedItem = Nothing Or UD_CB_CNIC.SelectedItem = Nothing Then
            MessageBox.Show("Please select ""Customer/CNIC No"".", "Message", MessageBoxButtons.OK, MessageBoxIcon.Hand)
        Else
            CustID = CusID(UD_CB_CName.SelectedItem, UD_CB_CNIC.SelectedItem)
            UD_TB_PrevBalance.Text = PrevBlnc(CustID)
        End If
    End Sub

    Private Sub UD_TB_TCredit_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub Btn_Update_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Update.Click
        Try
            If UD_CB_CName.SelectedItem = Nothing Then
                MessageBox.Show("Please select ""Customer's Name"".", "Message", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            ElseIf UD_CB_CNIC.SelectedItem = Nothing Then
                MessageBox.Show("Please select ""CNIC No"".", "Message", MessageBoxButtons.OK, MessageBoxIcon.Stop)
            ...
            Else
                ...
                Try
                    ...
                    cmd = New SqlCommand("SELECT MAX(Invoice_ID) As Invoice_ID FROM SR WHERE CID = '" & CustID & "'", con)
                    con.Open()
                    dr = cmd.ExecuteReader
                    ...
                    Using cmd As New SqlCommand
                        ...
                        Try
                            ...
                            If SelectionChangeCommited = True Then
                                cmd.CommandText = "UPDATE SRPayment SET Credit_Status = 'Active' " _
                                        & "WHERE Payment_ID = (SELECT MAX(SRPayment.Payment_ID) - 1 FROM " _
                                        & "SR INNER JOIN SRPayment ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE " _
                                        & "SR.CID = (SELECT SR.CID FROM SR INNER JOIN SRPayment ON SR.Invoice_ID " _
                                        & "= SRPayment.Invoice_ID WHERE(SRPayment.Payment_ID = '" & UD_TB_PID.Text.Trim() & "')))"
                                cmd.ExecuteNonQuery()
                            End If

                            If SelectionChangeCommited = True Then
                                cmd.CommandText = "Update SRPayment set Credit_Status = 'Idle' " _
                                    & "where Invoice_ID = (Select SR.Invoice_ID FROM SR INNER JOIN " _
                                    & "SRPayment ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE SR.CID " _
                                    & "= '" & CustID & "' AND GRNPayment.Credit_Status = " _
                                    & "'" & CStatus.Text & "')"
                                cmd.ExecuteNonQuery()
                            End If

                            cmd.CommandText = "Update SR set CID = '" & CustID & "',Invoice_No = '" & UD_TB_IN.Text.Trim() & "', Invoice_Date = '" & UD_DTP_Inv_Date.Text & "', Picture = @Picture  where Invoice_ID = '" & UD_TB_GRN.Text.Trim() & "'"
                            Dim ms As New MemoryStream()
                            ...
                            If CurrentRows < PreviousRows Then
                                For y = 0 To UD_DGV_Items.RowCount - 2
                                    ...
                                    cmd.CommandText = "UPDATE SRItems SET Item = '" & Itm & "', Meters = '" & Metrs & "', ItemTAmount = '" & ItmTAmnt & "' WHERE (Invoice_ID = '" & UD_TB_GRN.Text.Trim() & "') AND (Item_No = '" & INo & "')"
                                    cmd.ExecuteNonQuery()
                                Next
                                cmd.CommandText = "Delete from SRItems where (Invoice_ID = '" & UD_TB_GRN.Text.Trim() & "') And (Item_No > '" & CurrentRows & "')"
                                cmd.ExecuteNonQuery()
                            End If
    ...
    Private Sub Btn_D_Load_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_D_Load.Click
        ...
        Else
            ...
            cmd = New SqlCommand("Select * from SR where Invoice_ID = '" & TB_D_Load.Text.Trim() & "'", con)
            If con.State = ConnectionState.Closed Then con.Open()
            dr = cmd.ExecuteReader()
            If dr.Read() Then
                dr.Close()
                ...
                Try
                    ...
                    Dim CusmerID As Long = CIDByGRN(TB_D_Load.Text.Trim())
                    Using cmd = New SqlCommand("Select Invoice_ID from SR where Invoice_ID > '" & TB_D_Load.Text.Trim() & "' and CID = '" & CusmerID & "'", con)
                        If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                        Dim count As Integer = 0
                        Using dr = cmd.ExecuteReader()
                            If dr.Read() Then
                        ...
                        End Using
                    End Using
                    Using cmd = New SqlCommand("select * from SRItems where Invoice_ID = '" & TB_D_Load.Text.Trim() & "' and Return_Status = '" & RStas & "'", con)
                        If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                        Dim count As Integer = 0
                        Using dr = cmd.ExecuteReader()
                            If dr.Read() Then
                        ...
                        End Using
                    End Using
                    Using cmd = New SqlCommand("select Invoice_ID from SRPayment where Invoice_ID = '" & TB_D_Load.Text.Trim() & "'", con)
                        If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                        Dim count As Integer = 0
                        Using dr = cmd.ExecuteReader()
                            While dr.Read()
                    ...
                    Using cmd As New SqlCommand
                        ...
                        Try
                            cmd.CommandText = "Select * from SR where Invoice_ID = '" & TB_D_Load.Text.Trim() & "'"
                            Using dr = cmd.ExecuteReader()
                                If dr.Read() Then
                                ...
                                End If
                            End Using
                            cmd.CommandText = "Select * from SRItems where Invoice_ID = '" & TB_D_Load.Text.Trim() & "'"
                            Using dr = cmd.ExecuteReader()
                                While dr.Read
                                    ItmNo = dr("Item_No")
                                    Ietm = dr("Item")
                            ...
                            End Using
                            cmd.CommandText = "Select * from SRPayment where Invoice_ID = '" & TB_D_Load.Text.Trim() & "'"
                            Using dr = cmd.ExecuteReader()
                                If dr.Read() Then
                                    ...
                                    D_TB_GRNTA.Text = dr.Item("SRTAmount")
                            ...
                            With CNameCNICByCID(ID)
                                ...
                                CusmrCNIC = .Item2
                            End With
                            D_CB_CName.Text = CusmrName
                            D_CB_CNIC.Text = CusmrCNIC
    ...
    Private Sub D_TB_PrevBalance_Enter(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If D_CB_CName.Text = "" Or D_CB_CNIC.Text = "" Then
            MessageBox.Show("Please select ""Customer/CNIC No"".", "Message", MessageBoxButtons.OK, MessageBoxIcon.Hand)
        Else
            CustID = CusID(D_CB_CName.Text, D_CB_CNIC.Text)
            D_TB_PrevBalance.Text = PrevBlnc(CustID)
        End If
    End Sub
            ...
            Else
                Try
                    ...
                    Using cmd As New SqlCommand
                        ...
                        cmd.Transaction = transaction
                        Try
                            cmd.CommandText = "Delete from SRItems where Invoice_ID = '" & D_TB_GRN.Text.Trim() & "'"
                            cmd.ExecuteNonQuery()

                            cmd.CommandText = "Delete from SRPayment where Invoice_ID = '" & D_TB_GRN.Text.Trim() & "'"
                            cmd.ExecuteNonQuery()

                            cmd.CommandText = "Delete from SR where Invoice_ID = '" & D_TB_GRN.Text.Trim() & "'"
                            cmd.ExecuteNonQuery()

                            cmd.CommandText = "Update SRPayment SET Credit_Status = 'Active' WHERE Payment_ID = (" _
                                & "Select Max(SRPayment.Payment_ID) FROM SR INNER JOIN SRPayment " _
                                & "ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE SR.Invoice_ID = (" _
                                & "Select Max(SR.Invoice_ID) FROM SR INNER JOIN SRPayment " _
                                & "ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE SR.CID = '" & CustID & "' and " _
                                & "SRPayment.Credit_Status = 'Idle'))"
                            cmd.ExecuteNonQuery()
    ...
    Private Sub Invoice_ID()
        ...
        Dim Val As String
        Using cmd = New SqlCommand("select Invoice_ID from SR where Invoice_ID = (Select MAX(Invoice_ID) from SR)", con)
            If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
            Using dr = cmd.ExecuteReader
    ...
    Private Sub PmntID()
        Dim Val As String
        Using cmd = New SqlCommand("select Payment_ID from SRPayment where Payment_ID = (Select MAX(Payment_ID) from SRPayment)", con)
            If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
            Using dr = cmd.ExecuteReader
                If dr.HasRows = False Then
                    Val = 0
    ...
    Private Sub CusName()
        ...
        Try
            Using com As New SqlCommand("Select CName from Customers ORDER BY CName ASC", con)
                If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
    ...
    Function CusID(ByVal CName1 As String, ByVal CNIC1 As String) As Long
        '~~~~ Customer ID by Customer Name and CNIC ~~~~'

        CusID = 0
        Try
            Using com As New SqlCommand("Select CID from Customers where CName = '" & CName1 & "' and CNIC ='" & CNIC1 & "'", con)
                If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                Using dr = com.ExecuteReader
                    If dr.Read Then
                        CusID = dr.Item("CID")
    ...
    Function PrevBlnc(ByVal Cusidnty As Long) As Long
        PrevBlnc = 0
        Dim paimntId As String = 0
        Try
            Using com As New SqlCommand("SELECT SRPayment.Payment_ID FROM SR INNER JOIN SRPayment" _
                      & " ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE SR.CID = '" & Cusidnty & "' and " _
                      & "SRPayment.Credit_Status = '" & CStatus.Text & "'", con)
                If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                Using dr = com.ExecuteReader
                    If dr.Read Then
        ...
        If paimntId = UD_TB_PID.Text Then
            SelectionChangeCommited = False

            Dim naam, cnic As String
            naam = ""
            cnic = ""
            Try
                Using com As New SqlCommand("Select CName, CNIC from Customers where CID ='" & Cusidnty & "'", con)
                    If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                    Using dr = com.ExecuteReader
                        If dr.Read Then
                            naam = dr.Item("CName")
                            cnic = dr.Item("CNIC")
                        Else
                            MessageBox.Show("The Customer doesn't exist anymore. Please contact developer or add another Customer.", "message", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        End If
                    End Using
                End Using
            Catch ex As Exception
            ...
            Try
                Using com As New SqlCommand("SELECT TotalBalance " _
                                            & "FROM SRPayment " _
                                            & "WHERE (Payment_ID = " _
                                            & "(SELECT MAX(SRPayment_1.Payment_ID) AS Expr1 " _
                                            & "FROM SR INNER JOIN " _
                                            & "SRPayment AS SRPayment_1 ON SR.Invoice_ID = SRPayment_1.Invoice_ID " _
                                            & "WHERE (SRPayment_1.Credit_Status = 'Idle') AND (SR.CID = " _
                                            & "(SELECT CID " _
                                            & "FROM Customers " _
                                            & "WHERE (CName = '" & naam & "') " _
                                            & "AND (CNIC = '" & cnic & "')))))", con)
        ...
        Else
            Try
                Using com As New SqlCommand("SELECT SRPayment.TotalBalance FROM SR INNER JOIN SRPayment" _
                          & " ON SR.Invoice_ID = SRPayment.Invoice_ID WHERE SR.CID = '" & Cusidnty & "' and " _
                          & "SRPayment.Credit_Status = '" & CStatus.Text & "'", con)
                    If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                    Using dr = com.ExecuteReader
    ...
    Function CIDByGRN(ByVal GRNN As Long) As Long
        '~~~~ Custoemr ID by GRN-Number ~~~~'

        CIDByGRN = 0
        Try
            Using com As New SqlCommand("Select CID from SR where Invoice_ID = '" & GRNN & "'", con)
                If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                Using dr = com.ExecuteReader
                    If dr.Read Then
                        CIDByGRN = dr.Item("CID")
                    Else
                        CIDByGRN = 0
                    End If
                End Using
    ...
    Public Function CNameCNICByCID(ByVal ID As Integer) As Tuple(Of String, String)
        '~~~~ Customer ID by Customer Name and CNIC No ~~~~'
        Dim naam, cnic As String
        naam = ""
        cnic = ""
        Try
            Using com As New SqlCommand("Select CName, CNIC from Customers where CID ='" & ID & "'", con)
                If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                Using dr = com.ExecuteReader
                    If dr.Read Then
                        naam = dr.Item("CName")
        ...
        Catch ex As Exception
            MessageBox.Show(ex.Message)
        End Try
        Return New Tuple(Of String, String)(naam, cnic)
    End Function
    ...
    Private Sub CustCNIC()
        ...
        Try
            If caler.Name = "CB_CNIC_Enter" And CB_CName.SelectedItem <> Nothing Then
                Using com As New SqlCommand("Select CNIC from Customers where CName = '" & CB_CName.SelectedItem & "' ORDER BY CNIC ASC", con)
                    If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                    Using dr = com.ExecuteReader
                        CB_CNIC.Items.Clear()
                        While dr.Read
                    ...
                    End Using
                End Using
            ElseIf caler.Name = "UD_CB_CNIC_Enter" And UD_CB_CName.SelectedItem <> Nothing Or caler.Name = "Btn_UD_Load_Click" Then
                Using com As New SqlCommand("Select CNIC from Customers where CName = '" & UD_CB_CName.SelectedItem & "' ORDER BY CNIC ASC", con)
                    If con.State = ConnectionState.Closed Then con.Open() 'Connection Open
                    Using dr = com.ExecuteReader
                        UD_CB_CName.Items.Clear()
                        While dr.Read
